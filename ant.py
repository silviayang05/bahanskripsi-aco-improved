...
class Ant:
    ...
    def move_to_next_index(self, next_index):
        ...
        if self.graph.nodes[next_index].is_depot:
            # Jika lokasi berikutnya adalah titik server, muatan kendaraan, dll. harus dibersihkan.
            self.vehicle_load = 0
            self.vehicle_travel_time = 0

        else:
            # Perbarui beban kendaraan, jarak perjalanan, waktu
            self.vehicle_load += self.graph.nodes[next_index].demand
            # Jika lebih awal dari jangka waktu (ready_time) yang diminta oleh pelanggan, Anda harus menunggu
    ...
    def check_condition(self, next_index) -> bool:
        """
        Periksa apakah perpindahan ke poin berikutnya memenuhi batasan
        :param next_index:
        :return:
        """
        ...
        # Periksa apakah Anda dapat kembali ke bengkel servis setelah mengunjungi penumpang tertentu
    ...
    def cal_next_index_meet_constrains(self):
        """
        Temukan semua pelanggan yang dapat dijangkau dari lokasi saat ini (ant.current_index)
        :return:
        """
    ...
    def cal_nearest_next_index(self, next_index_list):
        """
        Pilih pelanggan yang paling dekat dengan lokasi saat ini (ant.current_index) dari pelanggan yang akan dipilih.
    ...
    def try_insert_on_path(self, node_id, stop_event: Event):
        """
        Coba masukkan node_id ke dalam travel_path saat ini
        Lokasi penyisipan tidak boleh melanggar batasan muatan, waktu, dan jarak perjalanan.
        Jika ada beberapa lokasi, temukan lokasi optimal
        :param node_id:
        :return:
        """
        ...
        for insert_index in range(len(self.travel_path)):
            ...
            # Temukan depot terdekat di depan insert_index
            ...
            # check_ant dimulai dari front_depot_index
            check_ant = Ant(self.graph, self.travel_path[front_depot_index])

            # Biarkan check_ant berjalan melalui titik di jalur di mana subskrip dimulai dari front_depot_index dan berakhir di insert_index-1
            for i in range(front_depot_index+1, insert_index):
                check_ant.move_to_next_index(self.travel_path[i])

            # Mulailah mengakses node secara tentatif dalam indeks_ke_visit yang diurutkan
            if check_ant.check_condition(node_id):
                check_ant.move_to_next_index(node_id)
            else:
                continue

            # Jika node_id dapat dijangkau, pastikan kendaraan dapat melakukan perjalanan kembali ke depo.
            for next_ind in self.travel_path[insert_index:]:
                ...
                # Jika tidak memungkinkan untuk kembali ke depo, kembali ke level sebelumnya
                else:
                    break
    ...
    def insertion_procedure(self, stop_even: Event):
        """
        Cobalah untuk menemukan lokasi yang cocok untuk setiap node yang belum dikunjungi dan masukkan ke dalam travel_path saat ini
        Lokasi penyisipan tidak boleh melanggar batasan muatan, waktu, dan jarak perjalanan.
        :return:
        """
        ...
        # Hingga tidak ada satu pun node yang belum dikunjungi yang berhasil disisipkan
        while success_to_insert:

            success_to_insert = False
            # Dapatkan node yang belum dikunjungi
            ind_to_visit = np.array(copy.deepcopy(self.index_to_visit))

            # Dapatkan permintaan untuk mengunjungi titik pelanggan, urutkan dalam urutan menurun
    ...
    def local_search_once(graph: VrptwGraph, travel_path: list, travel_distance: float, i_start, stop_event: Event):
        ...
        # Temukan lokasi semua depot di jalur
        depot_ind = []
        for ind in range(len(travel_path)):
            if graph.nodes[travel_path[ind]].is_depot:
                depot_ind.append(ind)

        # Bagilah self.travel_path menjadi beberapa segmen, setiap segmen dimulai dengan depot dan diakhiri dengan depot, yang disebut rute
                    ...
                    for end_a in range(start_a, min(depot_ind[i], start_a + 6)):
                        for start_b in range(depot_ind[j - 1] + 1, depot_ind[j]):
                            for end_b in range(start_b, min(depot_ind[j], start_b + 6)):
                                ...
                                # Tentukan apakah perubahan rute a layak dilakukan
                                ...
                                # Tentukan apakah perubahan rute b layak dilakukan
                                ...
                                if success_route_a and success_route_b:
                                    ...
                                    if new_path_distance < travel_distance:
                                        ...
                                        # Hapus salah satu depot yang terhubung bersama di jalur tersebut
    ...
    def local_search_procedure(self, stop_event: Event):
        """
        Gunakan tanda silang untuk melakukan pencarian lokal pada travel_path saat ini yang telah mengunjungi semua node dalam grafik.
        :return:
        """
